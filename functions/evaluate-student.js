      ...
      'https://api.openai.com/v1/chat/completions',
      {
        ...
        messages: [
          ...
          },
          {
            ...
            content: `
            #기본지침
            1. Text: ${text}에서 학생 목록을 파악한뒤, ${studentIndex}번째 순서의 학생의 이름과 평가점수(잘함,보통,노력요함 혹은 상,중,하)를 추출하고, 평가 기준: ${JSON.stringify(evaluationCriteria)}을 이해하고, 이에 따라 학생을 평가하는 자연스러운 문장을 생성해줘. 
            2. 학생이름은 {김,이,최,백,우,유,윤,박,노,강} 등의 단어로 시작하는 3글자 가량의 독립된 문자열이며, 텍스트에서 다음과 같이 번호(숫자)+빈칸+이름(문자열) 형태로 배열되어 있어. 
              1 이상선, 2 김선태, ... 이상한 문자열이 이름에 포함되지 않도록 정확한 이름을 추출하여 사용해줘. '전학생'문자열도 정확한 이름으로 간주해줘.
            3. 영역별 평가요소에 해당하는 문장을 기준으로 학생의 평가점수를 기준으로 학생의 수행 정도를 평가하는 구체적이고 자연스러운 문장으로 구성해줘. 
            4. 영역명을 직접 적지는 말아줘.
