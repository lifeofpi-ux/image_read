      ...
      'https://api.openai.com/v1/chat/completions',
      {
        ...
        messages: [
          ...
          },
          {
            ...
            2. #평가문장 예시 문장을 참고하여, 영역별 평가요소에 해당하는 문장을 기준으로 학생의 평가점수를 기준으로 학생의 수행 정도를 평가하는 구체적이고 자연스러운 문장으로 구성해줘. 
            ...
            ***다양한 상황과 대상에 따른 언어적 비언어적 표현의 효과에 대해 알고 실제 생활에 적용함. 글을 읽으면서 낱말의 뜻을 짐작해 보고 짐작한 뜻을 사전에서 찾아 확인함.
            ...
            형식: { "학생데이터": { "번호": "${studentIndex}", "이름": "${currentStudent.이름}", "평가점수": { ${evaluationAreas} } }, "평가결과": "..." }`
          }
        ],
...
exports.handler = async function (event, context) {
  ...
  try {
    ...
    if (!evaluationCriteria || studentIndex === undefined || !fullText || !tone || !wordCount || creativity === undefined) {
      return {
        statusCode: 400,
        body: JSON.stringify({ error: '잘못된 ��청 데이터입니다.' }),
      };
    }
